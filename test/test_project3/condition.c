...
int main(int argc, char *argv[])
{
	...
	*(int *)RESOURCE_ADDR = 0;
	...
	// Start producer
	char *argv_producer[5] = { "producer", buf_location, buf_cond_handle,
				   buf_lock_handle, buf_resource };
	pids[0] = sys_exec(argv_producer[0], 5, argv_producer);

	// Start consumers
	for (int i = 1; i < NUM_TC + 1; i++) {
		assert(itoa(print_location + i, buf_location, BUF_LEN, 10) !=
		       -1);

		char *argv_consumer[5] = { "consumer", buf_location,
					   buf_cond_handle, buf_lock_handle,
					   buf_resource };
