...
int main(int argc, char *argv[])
{
	...
	struct TestMultiCoreArg *singleCoreArg =
		(struct TestMultiCoreArg *)TEST_SINGLE_CORE;
	...
	printf("single core: %ld ticks, result = %d            \n\r",
	       singleCoreEnd - singleCoreBegin, single_core_result);

	struct TestMultiCoreArg *multiCoreArgs =
		(struct TestMultiCoreArg *)TEST_MULTI_CORE;
	pid_t pids[NUM_CPUS];
	...
	for (int i = 0; i < NUM_CPUS; ++i) {
		...
		assert(itoa((int)&multiCoreArgs[i], multicore_buf, BUF_LEN,
			    10) != -1);
	...
	printf("multi core: %ld ticks, result = %d             \n\r",
	       multiCoreEnd - multiCoreBegin, multi_core_final_result);
#endif
