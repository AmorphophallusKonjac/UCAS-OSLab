...
void fill_mailbox_id(char my_id)
{
	...
	for (id_i = 'a'; id_i < 'd'; ++id_i) {
		if (id_i != my_id) {
			...
			other_mailbox_id[idx][sizeof(mailbox_id_template) - 2] =
				id_i;
			other_id[idx] = id_i;
			++idx;
		}
...
void recv_thread(void *arg)
{
	...
	for (;;) {
		...
		int remain_size = DATA_LENGTH * 2 - bytes[other_id[0] - 'a'] -
				  bytes[other_id[1] - 'a'];
		if (len >= remain_size)
			len = remain_size;
		...
		printf("[%c-recv] from %c: %ld \t from %c: %ld\n", id,
		       other_id[0], bytes[other_id[0] - 'a'], other_id[1],
		       bytes[other_id[1] - 'a']);
		if (len == remain_size)
			break;
		sys_sleep(1);
...
void send_thread(void *arg)
{
	...
	for (;;) {
		len = rand() % (MAX_MBOX_LENGTH / 4 - 2) + 1;
		target = rand() % 2;
		if (bytes[target] >= DATA_LENGTH)
			target = 1 - target;
		if (len >= DATA_LENGTH - bytes[target])
			len = DATA_LENGTH - bytes[target];
