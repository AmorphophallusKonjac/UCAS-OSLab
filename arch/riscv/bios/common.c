...
{
	return call_bios((long)BIOS_GETCHAR, IGNORE, IGNORE, IGNORE, IGNORE,
			 IGNORE);
}
...
int sd_read(unsigned mem_address, unsigned num_of_blocks, unsigned block_id)
{
	return (int)call_bios((long)BIOS_SDREAD, (long)mem_address,
			      (long)num_of_blocks, (long)block_id, IGNORE,
			      IGNORE);
}
...
{
	call_bios((long)BIOS_SETTIMER, (long)stime_value, IGNORE, IGNORE,
		  IGNORE, IGNORE);
}

uint64_t read_fdt(enum FDT_TYPE type)
{
	return (uint64_t)call_bios((long)BIOS_FDTREAD, (long)type, IGNORE,
