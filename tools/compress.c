...
int main()
{
	...
	struct libdeflate_decompressor *decompressor =
		deflate_alloc_decompressor();
	char *compressed = (char *)malloc(nbytes_kernel + 10);
	memset(compressed, 0, nbytes_kernel + 10);

	/* do compress */
	int out_nbytes = deflate_deflate_compress(compressor, data,
						  nbytes_kernel, compressed,
						  nbytes_kernel + 10);
	...
	deflate_deflate_decompress(decompressor, compressed, out_nbytes,
				   extracted, 1000000, &restore_nbytes);
