...
static void get_compress_data(Elf64_Phdr phdr, char *data, int *data_len,
			      FILE *fp);
...
static void create_image(int nfiles, char *files[])
{
	...
	for (int fidx = 0; fidx < nfiles; ++fidx) {
		...
		if (strcmp(*files, "main") == 0) {
			...
			/* prepare environment */
			deflate_set_memory_allocator((void *(*)(int))malloc,
						     free);
			struct libdeflate_compressor *compressor =
				deflate_alloc_compressor(12);
			char *compressed = (char *)malloc(data_len + 10);
			memset(compressed, 0, data_len + 10);
			/* do compress */
			int out_nbytes = deflate_deflate_compress(
				compressor, data, data_len, compressed,
				data_len + 10);
			/* write back to main*/
			size_t bytes_write =
				fwrite(compressed, 1, out_nbytes, img);
		...
		if (taskidx >= 0) {
			taskinfo[taskidx].size =
				phyaddr - taskinfo[taskidx].offset;
		}
	...
	write_img_info(nbytes_kernel, nbytes_decompress, taskinfo, tasknum,
		       img);
			   ...
			   task_info_t *taskinfo, short tasknum, FILE *img)
{
	...
	uint16_t taskinfo_sector_num =
		(taskinfo_offset + taskinfo_size) / SECTOR_SIZE -
		taskinfo_sector_id + 1;
	...
	uint16_t kernel_sector_num =
		(kernel_offset + nbytes_kernel) / SECTOR_SIZE -
		kernel_sector_id + 1;
